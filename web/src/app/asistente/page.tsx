...
export default function AsistentePage() {
  ...
  return (
    <div className="flex flex-col h-full">
      {/* Selectores */}
      <div className="flex items-center justify-end gap-2 px-4 sm:px-6 lg:px-8 py-3 border-b border-slate-200/50">
        <div className="relative">
          <select
            value={selectedCrop}
            onChange={(e) => setSelectedCrop(e.target.value)}
            className="appearance-none px-3 py-2 pr-8 bg-white border border-slate-200 rounded-xl text-slate-900 text-sm focus:outline-none focus:border-green-500/50 cursor-pointer"
          >
        ...
        <div className="relative">
          <input
            ...
            placeholder="Sector"
            className="w-20 px-3 py-2 bg-white border border-slate-200 rounded-xl text-slate-900 text-sm focus:outline-none focus:border-green-500/50 placeholder-gray-500"
          />
        </div>
      </div>
          ...
          {messages.length === 0 ? (
            <div className="space-y-8">
              ...
              <div className="flex justify-center gap-3 flex-wrap">
                <div className="px-3 py-1.5 bg-white/50 border border-slate-200/50 rounded-lg text-xs text-slate-500">
                  ...
                  {suggestedQuestions.map((item, index) => {
                    ...
                    return (
                      <button
                        ...
                        className="flex items-start gap-3 p-4 bg-white/30 hover:bg-white/50 border border-slate-200/30 hover:border-green-500/30 rounded-xl text-left transition-all group"
                      >
                        <div className="p-2 rounded-lg bg-green-500/10 group-hover:bg-green-500/20 transition-colors">
                          <Icon className="w-5 h-5 text-green-400" />
                        </div>
                        <div>
                          <p className="text-slate-900 text-sm">{item.question}</p>
            ...
            <div className="space-y-6">
                  ...
                  <div className={`max-w-[90%] ${
                              ...
                              <div className="space-y-1 mb-2">
                                {protocol.products.map((prod, i) => (
                                  <div key={i} className="flex items-center gap-2 text-xs">
                                    <Beaker className="w-3 h-3 text-slate-500 shrink-0" />
                                    <span className="text-slate-900 font-medium">{prod.nombre}</span>
                                    <span className="text-slate-500">({prod.ingrediente_activo})</span>
                    ...
                    {message.sources && message.sources.length > 0 && (
                      <div className="mt-4 pt-3 border-t border-slate-200/30">
                        <p className="text-slate-500 text-xs mb-2">Fuentes:</p>
                        <div className="flex flex-wrap gap-2">
                          {message.sources.map((source, idx) => (
                            <span key={idx} className="inline-flex items-center gap-1 px-2 py-1 bg-white/50 rounded-lg text-slate-500 text-xs">
              ...
              {isLoading && (
                <div className="flex justify-start">
                  <div className="bg-white/30 border border-slate-200/30 rounded-2xl p-4">
      ...
      {/* Input Area */}
      <div className="sticky bottom-0 backdrop-blur-xl border-t border-slate-200/50">
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-4">
          ...
          <form onSubmit={handleSubmit} className="flex gap-3">
            ...
            <button
              ...
              disabled={isLoading}
              className="px-4 py-3 bg-white hover:bg-slate-100 border border-slate-200 rounded-xl transition-colors disabled:opacity-50"
            ...
            <input
              ...
              className="flex-1 px-4 py-3 bg-white/50 border border-slate-200 rounded-xl text-slate-900 placeholder-gray-500 focus:outline-none focus:border-green-500/50"
              disabled={isLoading}
            />
            <button
              type="submit"
