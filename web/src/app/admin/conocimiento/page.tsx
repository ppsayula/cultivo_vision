...
export default function ConocimientoPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Header */}
      <header className="sticky top-0 z-40 bg-slate-50/80 backdrop-blur-xl border-b border-slate-200">
        ...
        <div className="flex flex-col sm:flex-row gap-4 mb-6">
          <div className="flex-1 relative">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-gray-500 focus:outline-none focus:border-purple-500/50"
            />
          </div>
          <select
            value={filterCategory}
            onChange={(e) => setFilterCategory(e.target.value)}
            className="px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 focus:outline-none focus:border-purple-500/50"
          >
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70">
            <div className="bg-white rounded-2xl p-6 w-full max-w-2xl max-h-[90vh] overflow-y-auto">
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                <div>
                  <label className="block text-sm text-slate-500 mb-2">Titulo</label>
                  <input
                    type="text"
                    required
                    ...
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-gray-500 focus:outline-none focus:border-purple-500/50"
                  />
                </div>
                ...
                <div>
                  <label className="block text-sm text-slate-500 mb-2">Cultivos</label>
                  <div className="flex gap-3">
                    {CROP_TYPES.map(crop => (
                      ...
                      >
                        ...
                        <span className="text-slate-900">{crop.label}</span>
                      </button>
                    ))}
                  </div>
                </div>
                ...
                <div>
                  ...
                  <textarea
                    ...
                    className="w-full px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 placeholder-gray-500 focus:outline-none focus:border-purple-500/50 resize-none"
                  />
                </div>

                <div>
                  <label className="block text-sm text-slate-500 mb-2">Resumen (opcional)</label>
                  <textarea
                    rows={2}
                ...
                <div>
                  <label className="block text-sm text-slate-500 mb-2">Tags (separados por coma)</label>
                  <input
                    type="text"
                    value={formData.tags}
                ...
                <div className="flex gap-3 pt-4">
                  <button
                    ...
                    className="flex-1 px-4 py-3 bg-slate-50 border border-slate-200 rounded-xl text-slate-500 hover:bg-slate-100 transition-colors"
                  >
                    Cancelar
                  </button>
                  <button
                  ...
                  <div className="flex items-start justify-between gap-4">
                    <div className="flex-1">
                      <div className="flex items-center gap-3 mb-2">
                        ...
                        <span className="text-xs text-slate-500 uppercase">{catInfo.label}</span>
                      ...
                      <div className="flex flex-wrap gap-2 mt-3">
                        {doc.tags.slice(0, 5).map(tag => (
                          <span
                            key={tag}
                            className="px-2 py-0.5 bg-slate-50 rounded text-xs text-slate-500"
                          >
                            {tag}
                          </span>
                        ))}
                    ...
                    <div className="flex items-center gap-2">
                      <button
                        ...
                        className="p-2 rounded-lg bg-slate-50 hover:bg-slate-100 text-slate-500 hover:text-slate-900 transition-colors"
                      >
                        <Edit className="w-4 h-4" />
                      </button>
                      <button
                        onClick={() => handleDelete(doc.id)}
                        className="p-2 rounded-lg bg-slate-50 hover:bg-red-500/20 text-slate-500 hover:text-red-400 transition-colors"
                      >
